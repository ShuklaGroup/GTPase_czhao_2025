...
cl        = pickle.load(open('../cluster/cluster_ticas/clustering_tica_300.pkl', 'rb'))
cl_label  = cl.labels_
...
def extract(name, frame, id):
  ...
  print(trajname + '.xtc', top + '.gro')
  traj       = md.load(trajname + '.xtc', top=top + '.gro')[frame]
  ...
  os.system("cp " + path + "/ABI1-ROP11-" + str(p) + ".top " + opath + "/" + oname + ".top")

id = 1
for i in sorted(counts.values())[1:101]:
  state  = list(counts.values()).index(i)
  select = np.random.choice(range(0,len(frames[state])), size=4)
  frame  = frames[state][select[0]]
  extract(List[frame[0]], frame[1], id)
  id += 1
  frame  = frames[state][select[1]]
  extract(List[frame[0]], frame[1], id)
  id += 1
  frame  = frames[state][select[2]]
  extract(List[frame[0]], frame[1], id)
  id += 1
  frame  = frames[state][select[3]]
  extract(List[frame[0]], frame[1], id)
  id += 1
